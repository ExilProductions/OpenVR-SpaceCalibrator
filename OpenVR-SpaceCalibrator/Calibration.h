...
struct CalibrationContext
{
	...
	// HMD position tracking for detecting frozen tracking
	float xprev = 0.0f, yprev = 0.0f, zprev = 0.0f;

	float continuousCalibrationThreshold = 1.5f;
	...
	bool enableStaticRecalibration = false;
	bool lockRelativePosition = false;

	Eigen::AffineCompact3d refToTargetPose = Eigen::AffineCompact3d::Identity();
	bool relativePosCalibrated = false;

	// Shared memory for reading driver poses
	protocol::DriverPoseShmem poseShmem;
	vr::DriverPose_t driverPoses[vr::k_unMaxTrackedDeviceCount];
	...
	CalibrationContext() {
		calibratedScale = 1.0;
		memset(driverPoses, 0, sizeof(driverPoses));
		ResetConfig();
	}
	...
	void Log(const std::string &msg)
	{
		...
		// Linux doesn't have OutputDebugStringA, just print to cerr
	...
	bool TargetPoseIsValidSimple() const {
		return targetID >= 0 && targetID <= vr::k_unMaxTrackedDeviceCount
			&& driverPoses[targetID].poseIsValid && driverPoses[targetID].result == vr::ETrackingResult::TrackingResult_Running_OK;
	}

	bool ReferencePoseIsValidSimple() const {
		return referenceID >= 0 && referenceID <= vr::k_unMaxTrackedDeviceCount
			&& driverPoses[referenceID].poseIsValid && driverPoses[referenceID].result == vr::ETrackingResult::TrackingResult_Running_OK;
	}
};
