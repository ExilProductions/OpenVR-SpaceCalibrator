...
void BuildMenu(bool runningInOverlay)
{
	...
	if (CalCtx.state == CalibrationState::None)
	{
		...
		{
			if (ImGui::Button("Stop Continuous", ImVec2(width * scale, ImGui::GetTextLineHeight() * 2)))
			{
	...
	else
	{
		if (CalCtx.state == CalibrationState::Continuous)
		{
			ImGui::Button("Continuous calibration active...", ImVec2(ImGui::GetWindowContentRegionWidth(), ImGui::GetTextLineHeight() * 2));
		}
		else
		{
	...
	ImGui::Text("OpenVR Space Calibrator v" SPACECAL_VERSION_STRING " - by tach/pushrax - Linux Port by gyroninja - Continuous Calibration Port by Stavdel");
	if (runningInOverlay)
	{
		ImGui::SameLine();
...
void BuildSystemSelection(const VRState &state)
{
	...
	// Show indicator during continuous calibration
	bool disableSelection = (CalCtx.state == CalibrationState::Continuous);
	if (disableSelection)
	{
		ImGui::TextColored(ImColor(1.0f, 0.8f, 0.2f), "Device selection locked during continuous calibration");
	...
	// Only allow changes if not in continuous calibration
	if (!disableSelection && !referenceSystems.empty())
	{
	...
	else if (disableSelection && !referenceSystems.empty())
	{
		// Show combo but don't process changes
		int tempRef = currentReferenceSystem;
		ImGui::Combo("##ReferenceTrackingSystem", &tempRef, &referenceSystems[0], (int) referenceSystems.size());
	}
	else
	{
		// No systems available, show empty combo
		ImGui::Text("(No tracking systems)");
	}
	...
	// Only allow changes if not in continuous calibration
	if (!disableSelection && !targetSystems.empty())
	{
	...
	else if (disableSelection && !targetSystems.empty())
	{
		// Show combo but don't process changes
		int tempTarget = currentTargetSystem;
		ImGui::Combo("##TargetTrackingSystem", &tempTarget, &targetSystems[0], (int) targetSystems.size());
	}
	else
	{
		// No systems available, show empty text
		ImGui::Text("(No target systems)");
	}
...
void BuildDeviceSelections(const VRState &state)
{
	...
	// Disable selection during continuous calibration
	bool disableSelection = (CalCtx.state == CalibrationState::Continuous);
	if (disableSelection)
	{
		ImGui::PushStyleVar(ImGuiStyleVar_Alpha, ImGui::GetStyle().Alpha * 0.5f);
	...
	// Store device selection in temp variable if disabled
	int tempRefDevice = selectedRefDevice;
	BuildDeviceSelection(state, disableSelection ? tempRefDevice : selectedRefDevice, CalCtx.referenceTrackingSystem);
	if (!disableSelection)
	{
	...
	// Store device selection in temp variable if disabled
	int tempCalDevice = selectedCalDevice;
	BuildDeviceSelection(state, disableSelection ? tempCalDevice : selectedCalDevice, CalCtx.targetTrackingSystem);
	if (!disableSelection)
	{
